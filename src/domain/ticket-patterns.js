// ── TICKET PATTERNS ────────────────────────────────────────────────────────────
// NOTE (known deviation): getTicketHistory reads localStorage directly (via LS_HISTORY),
// which is not strictly "pure" for a domain-layer file. This is explicitly permitted
// per the task mapping for this file — flagged here and in the extraction report.
import { LS_HISTORY } from '../services/history-repo.js';

// Ticket history from saved snapshots
export function getTicketHistory(clientId) {
  try {
...
export function getTicketPattern(clientId) {
...
export function ticketSparkline(clientId) {
  const hist = getTicketHistory(clientId);
