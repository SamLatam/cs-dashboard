// src/ui/renovaciones.js
// Extracted verbatim from api/index.html (1).html — "REVENUE AT RISK PANEL",
// "RENEWAL MINI PANEL (en overview)", "CADENCE COVERAGE PANEL" and
// "PÁGINA RENOVACIONES COMPLETA" sections (source lines ~4273-4501).
//
// INTEGRATION NOTE: this file did not exist after the 4-agent extraction pass — none of the
// 4 tasks were assigned ownership of updateRenewalBadge/renderRevenueAtRisk/renderRenewalMini/
// renderCadenceCoverage/renderRenovaciones, even though ui/overview.js, ui/nav.js and
// services/client-state-repo.js all import from it (as `./renovaciones.js` / `../ui/renovaciones.js`).
// Created during integration by pulling the functions directly from the ground-truth source,
// matching the DOM ids already present in the rebuilt index.html shell (#revenueAtRiskPanel,
// #renewalMiniPanel, #cadenceCoveragePanel, #renovacionesContent, #nb-renovaciones) and the
// nav entry `nav-renovaciones` / page `page-renovaciones`.
...
import { fmtD, daysUntil, daysSince } from '../lib/format.js';
...
export function renderRevenueAtRisk(){
  const clients = getClients();
...
export function renderRenewalMini(){
  const clients = getClients();
...
export function renderCadenceCoverage(){
  const clients = getClients();
...
export function renderRenovaciones(){
  const clients = getClients();
