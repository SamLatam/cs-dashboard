import { getClients, activeClients, PORT } from '../services/clients-repo.js';
import { getActions } from '../services/actions-repo.js';
import { getActiveUser, loadUsers, getCsmFilter } from '../services/profiles-repo.js';
...
import { LC_STAGES } from '../domain/lifecycle.js';
import { getLifecycle, moodBadge } from '../services/client-state-repo.js';
import { fmtG, fmtD, daysSince, norm } from '../lib/format.js';
import { tipoChamadoBadge, presenciaReunBadge, segPorteRow, showToast } from '../lib/dom.js';
// ASSUMPTION: LS_HISTORY forward-referenced from services/history-repo.js, matching the
// pattern already established by domain/ticket-patterns.js (that file was not yet created at
// extraction time either, per that agent's own note).
import { LS_HISTORY } from '../services/history-repo.js';
import { renderKPIs } from './kpis.js';
// NOTE: circular import with nav.js (nav.js imports renderOverview/renderTable from this file).
// Safe because `nav` is only invoked at click-time inside filterByLifecycle(), never during this
// module's top-level evaluation — same pattern already used/documented for startApp() in
// services/auth.js (see CONVENTIONS.md).
import { nav } from './nav.js';
// ASSUMPTION: renderRevenueAtRisk / renderRenewalMini / renderCadenceCoverage were not in this
// agent's explicit function list for overview.js (only renderLifecyclePipeline was). They are
// called from renderOverview() and, per source, live in adjacent but separate sections
// ("REVENUE AT RISK" / "RENEWAL MINI PANEL" / "CADENCE COVERAGE PANEL"). Best-guess home is
// ui/renovaciones.js (that file doesn't exist yet; a domain/cadence.js already exists from
// another agent, suggesting a ui counterpart is expected somewhere — flagged for reconciliation).
import { renderRevenueAtRisk, renderRenewalMini, renderCadenceCoverage } from './renovaciones.js';

// ── STATE (overview-page filter/sort, module-local) ───────────────────────────
let sortKey = 'hs', sortDir = -1;
let activeFilter = 'todos';

// ── RISK BANNER ────────────────────────────────────────────────────────────────
export function renderRiskBanner() {
  const container = document.getElementById('risk-banner-container');
  if (!container) return;
  const atRisk = getClients().filter(c => getRiskSignals(c).some(s => s.type==='health'||s.type==='nps'));
...
export function renderOverviewRisk() {
...
export function buildDonut(svgId, legendId, dist, colors) {
...
export function renderTable() {
  ...
  let _filteredClients = [...getClients()];
  if (_isDirector) {
  ...
  tbody.innerHTML=list.map(c=>{
    ...
    const actCnt=getActions().filter(a=>a.clientId===c.id&&a.status!=='resuelto').length;
    const actBl=getActions().filter(a=>a.clientId===c.id&&a.status==='bloqueado').length;
  ...
  if(_cancCount)_cancCount.textContent=getClients().filter(c=>c.weekly&&c.weekly.cancelado).length||'';
}
export function sortBy(k){if(sortKey===k)sortDir*=-1;else{sortKey=k;sortDir=k==='name'?1:-1;}document.querySelectorAll('th').forEach(t=>{t.classList.remove('sorted');const i=t.textContent.replace(/[↑↓↕]/g,'').trim();t.textContent=i+' ↕';});const th=document.getElementById('th-'+k);if(th){th.classList.add('sorted');th.textContent=th.textContent.replace('↕',sortDir===1?'↑':'↓');}renderTable();}
export function setFilter(f,el){activeFilter=f;document.querySelectorAll('.chip').forEach(c=>c.classList.remove('active'));el.classList.add('active');renderTable();}

// ── LIFECYCLE PIPELINE (overview panel) ───────────────────────────────────────
export function renderLifecyclePipeline(){
...
export function filterByLifecycle(lc){
