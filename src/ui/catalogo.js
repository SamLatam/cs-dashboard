// ── CATÁLOGO & GMV ─────────────────────────────────────────────────────────────
// Extracted verbatim from index.html (source lines 2880-2992).
//
// ASSUMED CROSS-FILE IMPORTS (sibling files did not exist yet at extraction time):
//   - getClients      from '../services/clients-repo.js'
//   - fmtG            from '../lib/format.js'
//   - lastZdFetch     from '../services/zendesk.js'  -- a mutable `let lastZdFetch = null`
//     in source (declared near autoFetchAll), reassigned elsewhere on each successful
//     sync. Imported as a live binding read here; NOT reassigned by this file.
import { getClients } from '../services/clients-repo.js';
import { fmtG } from '../lib/format.js';
import { lastZdFetch } from '../services/zendesk-sync.js';

// Source lines 2880-2992
...
// No window-attach needed for renderCatalogo itself (only ever invoked from
// nav()). Note: the HTML it produces references autoFetchAll/openCfgModal/
// openImport/exportarJSON/editMktInline/saveMktInline/closeMktInline/nav/
// renderFicha purely as onclick="..." strings — those are resolved at click
// time via whichever module window-attaches them (editMktInline/saveMktInline/
// closeMktInline are this agent's own, defined in ficha.js; the rest belong to
// other files) and require no import here since they're never called as bare
// JS identifiers in this file.
