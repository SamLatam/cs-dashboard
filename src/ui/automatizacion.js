// Extracted verbatim from index.html (lines ~5423-5648, "AUTOMATIZACIÓN D/E")
//
// NOTE: `_churnProb`/`_priorityScore` below are a churn scorer that is SEPARATE
// from `calcChurnRisk` in domain/predictions.js (written by another agent) —
// these are two genuinely different functions in the source and must not be
// merged.
//
// getCadenceStage/getCadenceInfo (originally right after this block, at
// index.html:5604-5605) were extracted into ../domain/cadence.js by another
// agent and are imported here since renderAutoCard calls them.
//
...
import { showToast, segBadge, porteBadge, tipoChamadoBadge, presenciaReunBadge, tendenciaLabel } from '../lib/dom.js';

function _churnProb(x){
...
function _priorityScore(x){return Math.round((100-x.hs)*0.6+_churnProb(x)*0.4);}
function getHSSparkline(clientId){
  try{
...
function _getCCORec(c,w,tier,cp,causes,tips){
...
function renderAutoCard(x,tier,rank){
...
function generateAutoMessages(c,w,tier,tips,causes,hsScore,cp){
...
function showAutoMsg(cId,idx){[0,1,2,3].forEach(function(i){var p=document.getElementById('amsg_'+cId+'_'+i);var b=document.getElementById('abtn_'+cId+'_'+i);if(p)p.style.display=i===idx?'block':'none';if(b){b.style.background=i===idx?'var(--accent)':'transparent';b.style.color=i===idx?'#fff':'var(--text)';b.style.border=i===idx?'none':'1px solid var(--border)';}});}
function copyAutoMsg(panelId){var el=document.getElementById(panelId);if(!el)return;var txt=el.querySelector('div').textContent||'';navigator.clipboard.writeText(txt).then(function(){showToast('📋 Copiado al portapapeles','green');}).catch(function(){showToast('Selecciona el texto manualmente','orange');});}
function logAutoContact(clientId){var today=new Date().toISOString().split('T')[0];var c=getClients().find(function(x){return x.id===clientId;});if(c){c.weekly=c.weekly||{};c.weekly.lastContact=today;saveData();showToast('✓ Contacto registrado — '+c.name,'green');renderAutomatizacion();}}

// The auto card template built in renderAutoCard() references showAutoMsg,
// copyAutoMsg and logAutoContact via inline onclick="" attributes, so they
// must be reachable from global scope.
...
export {
  _churnProb, _priorityScore, getHSSparkline, _getCCORec, _todayActions,
  renderAutomatizacion, getAutoTriggers, renderAutoCard,
  generateAutoMessages, showAutoMsg, copyAutoMsg, logAutoContact
};
