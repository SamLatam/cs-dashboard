// src/ui/director.js
// Extracted verbatim from api/index.html (1).html — director dashboard + team management.
//
// NOTE (assumption, not explicitly enumerated in the task spec): the source mutates the
// module-level `_dirCurrentWeek` variable (owned by notes-repo.js per task instructions,
// "SKIP, notes-repo.js's") via a bare reassignment (`_dirCurrentWeek = weekLabel;`). Per the
// shared CONVENTIONS.md rule for module-level mutable state (same as clients/actions), a bare
// reassignment of an imported binding is not legal in ES modules, so this file calls
// `setDirCurrentWeek(weekLabel)`, assuming notes-repo.js exports a getter/setter pair
// (`getDirCurrentWeek`/`setDirCurrentWeek`) analogous to the clients/actions accessor pattern.
// This wasn't in the explicit notes-repo.js export list given — flagged for integration check.

import { loadUsers, getActiveUser } from '../services/profiles-repo.js';
import { saveDirNote, DIR_NOTES_KEY, setDirCurrentWeek } from '../services/notes-repo.js';
import { fromSeed, PORT } from '../services/clients-repo.js';
